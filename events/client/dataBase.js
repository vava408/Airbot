...
module.exports = {
	...
	run(client) 
	{
		...
		//bienvenue Enrevoir
		const bienvenueEnrevoirTable = sql.prepare("SELECT count(*) FROM sqlite_master WHERE type='table' AND name = 'bienvenue';").get();
		if (!bienvenueEnrevoirTable['count(*)'])
		{
			sql.prepare("CREATE TABLE bienvenue (guild TEXT PRIMARY KEY, channel TEXT);").run();
		};
	},
};
